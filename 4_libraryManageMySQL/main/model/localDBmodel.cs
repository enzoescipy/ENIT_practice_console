...
namespace MainProject
{
        ...
        where VO : BasicVO , new()
    {
        ...
        public LocalDBmodel(MySqlConnection mySqlConnection, string myTableName)
        {
            ...
            this.voFieldsInfoExceptPkey  = (from vo in voFieldsInfo where !String.Equals(vo.Name, "primaryKey") select vo).ToArray();
        }
        ...
        public int Append(List<VO> newVOList)
        {
            ...
            {
                string valueQuery = "( " + String.Join(",",from voInfo in voFieldsInfoExceptPkey select $"`{voInfo.GetValue(vo)}`") + " );";
        ...
        public int Override(List<VO> newVOList)
        {
            ...
            {
                string valueQuery = "( " + String.Join(",",from voInfo in voFieldsInfoExceptPkey select $"`{voInfo.GetValue(vo)}`") + " ) ";
                string pkeyQuery = $"WHERE (`pkey` = {vo.primaryKey});";
                queryList.Add(updateQuery + valueQuery + valueQuery);
            }
