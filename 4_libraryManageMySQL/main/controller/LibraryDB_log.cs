...
namespace MainProject
{
    ...
    {   
        ...
        public int LogValidate(LogVO logVO)
        {
            ...
            var userQ = localDBmodel_user.Find("primaryKey", logVO.userPKey, 0);
            if (userQ.Count == 0) { return 1; }
            var userVO = userQ[0];

            // validate if bookPKey exists.
            var bookQ = localDBmodel_book.Find("primaryKey", logVO.bookPKey, 0);
        ...
        public List<List<string>> ListVOStringify(List<LogVO> voTable)
        {
            ...
            foreach(LogVO log in voTable)
            {
                ...
                var user = localDBmodel_user.Find("primaryKey", log.userPKey, 0)[0];
                var book = localDBmodel_book.Find("primaryKey", log.bookPKey, 0)[0];
                line.Add(user.email);
                line.Add(book.name);
        ...
        public List<dynamic[]> UserBorrowedFind(int userPkey)
        {
            ...
            for (int i=0; i < logOrderList.Count; i++)
            {
                ...
                {
                    var bookQ = localDBmodel_book.Find("primaryKey", bookPkeyList[i], 0);
        ...
        public List<dynamic[]>? BookBorrowedFind(int bookPkey)
        {
            ...
            for (int i=0; i < logOrderList.Count; i++)
            {
                if (returnLeftList[i] != 0)
                {
                    var userQ = localDBmodel_user.Find("primaryKey", userPkeyList[i], 0);
        ...
        public int DeleteLogForBook(string bookName)
        {
            ...
            var logQ = localDBmodel.Find("bookPKey", bookQ[0].primaryKey, 0);
