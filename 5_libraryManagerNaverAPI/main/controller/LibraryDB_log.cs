...
namespace MainProject
{
    ...
    {   
        ...
        public List<LogVO> LogSearch(int userPKey, int bookPKey)
        {
            ...
            var bookPkeySearched = localDBmodel.Find("bookPKey", bookPKey, 0);
        ...
        public int LogValidate(LogVO logVO)
        {
            ...
            var userQ = localDBmodel_user.Find("pkey", logVO.userPKey, 0);
            if (userQ.Count == 0) { return 1; }
            var userVO = userQ[0];

            // validate if bookPKey exists.
            var bookQ = localDBmodel_book.Find("pkey", logVO.bookPKey, 0);
        ...
        public List<List<string>> ListVOStringify(List<LogVO> voTable)
        {
            ...
            foreach(LogVO log in voTable)
            {
                ...
                var user = localDBmodel_user.Find("pkey", log.userPKey, 0)[0];
                var book = localDBmodel_book.Find("pkey", log.bookPKey, 0)[0];
                line.Add(user.email);
                line.Add(book.name);
        ...
        public List<dynamic[]> UserBorrowedFind(int userPkey)
        {
            ...
            var query = localDBmodel.Find("userPKey", userPkey, 0);
            ...
            for (int i=0; i < logOrderList.Count; i++)
            {
                ...
                {
                    var bookQ = localDBmodel_book.Find("pkey", bookPkeyList[i], 0);
        ...
        {
            // search for the all user who  had borrowed that book.
            var query = localDBmodel.Find("bookPKey", bookPkey, 0);
            if (query.Count == 0) { return new List<dynamic[]>(); }
            ...
            for (int i=0; i < logOrderList.Count; i++)
            {
                if (returnLeftList[i] != 0)
                {
                    var userQ = localDBmodel_user.Find("pkey", userPkeyList[i], 0);
        ...
        public int DeleteLogForBook(string bookName)
        {
            ...
            var logQ = localDBmodel.Find("bookPKey", bookQ[0].pkey, 0);
            var deleteTargetLogs = (from log in logQ select log.pkey).ToList();
