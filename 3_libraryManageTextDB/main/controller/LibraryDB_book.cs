...
{
    public class LibraryDB_Book : LibraryDBPart
    {
        ...
        {
            // find the keyword in the DB
            var nameQuery = localDBmodel.Find("name", name, stringEquals);
        ...
        {
            // find pKey book
            var query = localDBmodel.Find("name", targetName, stringEquals);
            if (query.Count == 0) { return -1; }
        ...
        // {
        //     // find pKey book
        //     var query = localDBmodel.Find("name", booktName, stringEquals);
        //     if (query.Count == 0) { return -1; }
        //     else if (query.Count > 1) { throw new InvalidDataException("primaryKey-matching VO must be unique."); }
